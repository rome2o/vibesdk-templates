...
# Setup script for the Shopify Polaris Cloudflare Durable Object template
...
NEW_PROJECT_NAME="$1"
echo "Setting up Shopify Polaris Durable Object project: $NEW_PROJECT_NAME"
...
if [[ "$OSTYPE" == "darwin"* ]]; then
    # macOS sed syntax
    sed -i '' "s/\"name\": \"shopify-polaris-runner\"/\"name\": \"$NEW_PROJECT_NAME\"/g" wrangler.jsonc
else
    # Linux sed syntax
    sed -i "s/\"name\": \"shopify-polaris-runner\"/\"name\": \"$NEW_PROJECT_NAME\"/g" wrangler.jsonc
fi
...
echo "2. bun run dev (to start development server)"
echo "3. bun run deploy (when ready to deploy)"